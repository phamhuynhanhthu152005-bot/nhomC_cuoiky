...
class HrPayslip(models.Model):
    ...
    # In Odoo 19 Community, hr.contract is replaced by hr.version
    ...
    refunded_id = fields.Many2one(
        "hr.payslip", string="Refunded Payslip"
    )
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        """
        @param contracts: Browse record of hr.version (contracts)
    ...
    def _get_salary_rules(self):
        "Return rules for the Payslips, sorted by sequence"
    ...
    def get_payslip_vals(
        ...
        if not self.env.context.get("contract"):
            # Get current active version for employee
            contract_ids = employee.version_id.ids if employee.version_id else []
        else:
            if contract_id:
                contract_ids = [contract_id]
            else:
                contract_ids = self._get_active_versions(employee, date_from, date_to).ids
        if not contract_ids:
            return res
    ...
    def _get_active_versions(self, employee, date_from, date_to):
        """Get active hr.version records for an employee during a period."""
